...
export async function getArticle(title) {
    try {
        const article = await findOne({ title }, { createdAt: -1, updatedAt: -1 })
        if (!article)
            return { error: 'Makele Bulunamadı.' }
...
export async function getAllArticleTitles() {
    try {
        const articles = await find({}, { _id: 0, title: 1 })

        for (let i = 0; i < articles?.length; i++)
            articles[i] = getDashedString(articles[i].title)

        return { articleTitles: articles }
    } catch (error) {
        return { error: error.message }
    }
}
