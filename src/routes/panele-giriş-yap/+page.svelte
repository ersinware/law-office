<script>
    ...
    } from "$lib/js/client/common/util.validations.client.common.js";
    ...
    async function handleSearchParam() {
        ...
        switch (decodeURI($page.url.search.substring(1))) {
            case "yetkisiz":
                showErrorSnackbar(
                    "Bu sayfayı görüntülemek için yetkiniz yok. Lütfen giriş yapın.",
                );
                break;
        }
    }
...
<svelte:head>
    ...
    <title
        >Panele Giriş Yap | Mirici & Partners Avukatlık Bürosu | Merkez, Çanakkale</title
    >
</svelte:head>

<section
    class="wrapper flex f-column a-i-c j-c-c h-100vh p-h-d"
    in:fly={TRANSITION_PAGE}
>
    <form
        class="modal-content-wrapper p-r flex f-column a-i-c modal-g-v-d secondary-background-color t-a-c b-r-d o-hidden big-screen-small-modal-content-wrapper-w small-screen-b-box small-screen-w-100 small-screen-max-w-phone-with-padding-h"
        bind:this={form}
    >
